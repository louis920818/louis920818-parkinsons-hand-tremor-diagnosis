...
TASKS=os.environ.get("TASKS","LiftHold").split(",")
for sid in sorted(lab.keys()):
    for task in TASKS:
        for wr in ["LeftWrist","RightWrist"]:
            p=os.path.join(TS,f"{sid}_{task}_{wr}.txt")
            if not os.path.exists(p): continue
            r=load(p)
            if r is None: continue
            acc,gyr=r; N=acc.shape[0]; wi=0
            for st in range(0,N-win+1,hop):
                f=win_feats(acc[st:st+win],gyr[st:st+win])
                if f is None: continue
                rows.append([sid,lab[sid],task,wr,wi]+f); wi+=1
df=pd.DataFrame(rows,columns=["sid","label","task","wrist","win"]+COLS)
out=os.path.expanduser(os.environ.get("OUT","pads_win_chunk.csv")); df.to_csv(out,index=False)
